...
"""
import os, json, datetime, google.auth
...
INPUT_TEMPLATE  = f"projects/{PROJECT_ID}/locations/{MA_LOCATION}/templates/input-prompt-template"
OUTPUT_TEMPLATE = f"projects/{PROJECT_ID}/locations/{MA_LOCATION}/templates/output-prompt-template"
...
def retrieve(q, k=3):
    ...
    p = [bigquery.ScalarQueryParameter("q", "STRING", q),
         bigquery.ScalarQueryParameter("k", "INT64", k)]
    return [dict(r) for r in bq.query(sql,
        job_config=bigquery.QueryJobConfig(query_parameters=p)).result()]

def log_interaction(q, bi, bo, rows, resp):
    bq.insert_rows_json(LOG_TABLE, [{
        "ts": datetime.datetime.utcnow().isoformat(), "question": q,
        "blocked_input": bi, "blocked_output": bo,
